...
#########################
method = "RandomCV"
traindataset = "MS-UMG_wapred_MS-UMG"
...
def tune_and_evaluate(
    ...
    seed=None,
    thresholds=None,
    get_shap_values_fn=None,
    auto_threshold=False
):
    """
    1) Tune hyperparameters on (X_train, y_train) via cross-validation.
    2) Retrain final model on entire (X_train, y_train) with best hyperparams.
    3) Evaluate final model on both (X_train, y_train) and (X_test, y_test) 
       for each threshold, storing both sets of metrics.
    """
    ...
    print(f"[Seed={seed}] Best params from training CV: {best_params}")
    print(f"[Seed={seed}] Corresponding CV score: {best_score}")

    final_model = LogisticRegression(random_state=seed, **best_params)
    final_model.fit(X_train, y_train)

    # Probabilities + SHAP values
...
if __name__ == "__main__":
    ...
    sub_dir = os.path.join(f"./results/train{traindataset}test{testdataset}/", f"lr_{run_id}_{method}_itercall{n_iter_call}")
    ...
    for seed in seeds:
        ...
        results_for_seed, final_model = tune_and_evaluate(
            ...
            get_shap_values_fn=get_separated_shap_values_log,
            auto_threshold=True
        )
        ...
        print(f"[Seed={seed}] Positive-class area under Precision-Recall (TEST): {test_ap:.4f}")
        print(f"[Seed={seed}] Positive-class area under Precision recall (TRAIN): {train_ap:.4f}")
